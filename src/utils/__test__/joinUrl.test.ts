...
it('should return url string because url is absolute path', () => {
  ...
  expect(joinedUrl.href).toBe('https://www.y.com/');
});
...
it('should concat url with baseURL even if we use URL object', () => {
  ...
  expect(joinedUrl.href).toBe('https://www.x.com/api/user?id=3');
});
...
it('should join url with searchParams', () => {
  ...
  joinedUrl = joinUrl(new URL('https://www.y.com/api'), httpOptions);
  expect(joinedUrl.href).toBe(
    'https://www.y.com/api?name=pranshu&lastname=shah&age=25'
  );
});
